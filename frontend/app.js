function getshitdone(argument) {
	...
	$.ajax({
		  ...
		  success: function(data) {
		  		var btntext = data.completed ? "UnDone" : "Done";
		  ...
		  success: function(data) {
		  	data.objects.forEach(function(item){
		  		if (item.subtask != '' && !item.sub_task) {
		  			var btntext = item.completed ? "UnDone" : "Done";
		  			$(".toAppend").append("<li class='dir task_id"+item.id+"'' task_id='"+item.id+"'>"+ item.title +"("+ item.due_date +")<button task_id='"+item.id+"' onclick='getshitdone(this)'>"+btntext+"</button><br><input type='text' placeholder='Add subtask' parent_task='"+item.id+"' name='subtask' class='addsubtask'><input type='date' id='date'><ul></ul></li>");
		  			...
		  			var btntext = item.completed ? "UnDone" : "Done";
		  			$(".toAppend").append("<li class='dir task_id"+item.id+"'' task_id='"+item.id+"'>"+ item.title +"("+ item.due_date +")<button task_id='"+item.id+"' onclick='getshitdone(this)'>"+btntext+"</button><br><input type='text' placeholder='Add subtask' parent_task='"+item.id+"' name='subtask' class='addsubtask'><ul></ul></li>");
		  		}
		  	});
		  },
		  error: function(data) {
...
function addNewTodo(data) {
	$.ajax({
		  ...
		  contentType: "application/json;",
		  success: function(data) {
		  		$(".toAppend").append("<li class='dir task_id"+data.id+"'' task_id='"+data.id+"'>"+ data.title +"("+ data.due_date +")<button task_id='"+data.id+"' onclick='getshitdone(this)'>Done</button><br><input type='text' placeholder='Add subtask' parent_task='"+data.id+"' name='subtask' class='addsubtask'><ul></ul></li>");
		  },
		  error: function(data) {
			successmessage = 'Error';
		  },
